...
public class Robot extends IterativeRobot {
	...
	TalonSRX leftTalon = new TalonSRX(14);

	/** some example logic on how one can manage an MP */
	MotionProfileExample rightProf = new MotionProfileExample(rightTalon);
	MotionProfileExample leftProf = new MotionProfileExample(leftTalon);
	...
	public void disabledInit() {
		...
		rightTalon.config_kF(0,.1981, 0);
		rightTalon.config_kP(0, 1.8, 0);
		rightTalon.config_kI(0, 0, 0);
		rightTalon.config_kD(0, 90, 0);
		
		leftTalon.config_kF(0, 2.3499, 0);
		leftTalon.config_kP(0,2.2, 0);
	...
	public void teleopPeriodic() {
		...
		if (btns[5] == false) {
			...
			leftTalon.set(ControlMode.MotionProfile, leftYjoystick);
			rightTalon.set(ControlMode.MotionProfile, leftYjoystick);
			...
			SetValueMotionProfile setOutputL = leftProf.getSetValue();
			SetValueMotionProfile setOutputR = rightProf.getSetValue();
			
			leftTalon.set(ControlMode.MotionProfile, setOutputL.value);
			rightTalon.set(ControlMode.MotionProfile, setOutputR.value);
			/*
			...
			if ((btns[6] == true) && (_btnsLast[6] == false)) {
				...
				leftProf.startMotionProfile();
				rightProf.startMotionProfile();
			}
		}

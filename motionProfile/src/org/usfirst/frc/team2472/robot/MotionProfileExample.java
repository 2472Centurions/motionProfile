...
public class MotionProfileExample {
	 ...
	 * keep one copy.
	 */
	GeneratedMotionProfile gen=new GeneratedMotionProfile();
	...
	public void control(int number) {
		...
		if (_talon.getControlMode() != ControlMode.MotionProfile) {
			...
			/*
			 * we are not in MP mode. We ;r4are probably driving the robot around
	...
	private void startFilling(int number) {
		/* since this example only has one talon, just update that one */
		
		if(number==1)startFilling(GeneratedMotionProfile.PointsR, GeneratedMotionProfile.kNumPoints,number);
		if(number==0)startFilling(GeneratedMotionProfile.pointsL,GeneratedMotionProfile.kNumPoints,number);
	}
	
	

	private void startFilling(double[][] profile, int totalCnt,int number) {
		...
		for (int i = 0; i < totalCnt; ++i) {
			...
			if(number==1) {velocityRPM=(velocityRPM*3725)/600;
							positionRot=(positionRot*3725)/600;
			}
